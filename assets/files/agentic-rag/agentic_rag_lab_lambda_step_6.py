#!/usr/bin/env python3
"""
Custom agentic RAG orchestration for Amazon Bedrock Knowledge Bases.

Milestone: after Part 10 Step 6 (final lab version).
- Step 2 adds an optional Bedrock Guardrail to the final answer call.
- Step 3 emits RAG-specific CloudWatch metrics via Embedded Metric Format.
- Step 6 adds graceful degradation: sources-only and "not enough evidence"
  fallbacks, plus a FallbackTriggered metric.

For the intermediate version without the Step 6 fallback behavior, see
agentic_rag_lab_lambda_steps_2_3.py.
...
5. Apply fallback behavior when retrieval or synthesis is weak.
...
def emit_rag_metrics(top_score, used_agentic_path, no_useful_chunks, fallback_triggered):
    print(json.dumps({
        "_aws": {
...
def format_sources_only(chunks: List[Dict[str, Any]]) -> str:
    lines = ["I found some potentially relevant content but the evidence is not strong "
             "enough for a confident answer. Here are the closest sources:\n"]
...
def run_agentic_rag(question: str) -> Dict[str, Any]:
    ...
    # No chunks at all -> explicit "not enough evidence"
    if no_useful_chunks:
        fallback_triggered = True
        answer = "I could not find enough relevant evidence in the knowledge base to answer this question."
    # Weak retrieval -> return sources without full synthesis
    elif top_score < WEAK_SCORE_THRESHOLD:
        fallback_triggered = True
        answer = format_sources_only(unique_chunks)
    # Normal path -> full synthesis
    else:
